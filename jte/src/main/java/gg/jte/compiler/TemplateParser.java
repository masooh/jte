...
public final class TemplateParser {
      ...
      for ( i = startIndex; i < endIndex; ++i ) {
         ...
         if ( !currentMode.isComment() && regionMatches("@import") && isParamOrImportAllowed() ) {
            ...
            if ( currentMode == Mode.Text ) {
               ...
               lastIndex = i + 1;
               push(Mode.Text);
            } else if ( currentMode == Mode.ForIndexLoop ) {
               visitor.onCodeStatement(depth, "int index = 0");
               extract(templateCode, lastIndex, i, visitor::onForLoopStart);
               lastIndex = i + 1;
               push(Mode.Text);
         ...
         } else if ( currentMode == Mode.Text && regionMatches("@forIndex") ) {
            extractTextPart(i - "forIndex".length(), Mode.ForIndexLoop);
            lastIndex = i + 1;
            push(Mode.ForIndexLoop);
         } else if ( currentMode == Mode.Text && regionMatches("@for") && nextChar() != 'I' ) {
            ...
            if ( currentMode == Mode.ForIndexLoop ) {
               visitor.onCodeStatement(depth, "index++");
            }
            if ( currentMode == Mode.ForLoop || currentMode == Mode.ForIndexLoop ) {
   ...
   private void handleUnclosedKeywords() {
      ...
      } else if ( currentMode == Mode.ForLoop || currentMode == Mode.ForIndexLoop ) {
   ...
   private boolean isContentExpressionAllowed() {
      return currentMode == Mode.Content || currentMode == Mode.JavaCodeParam || currentMode == Mode.Code || currentMode == Mode.CodeStatement
            || currentMode == Mode.Param;
   }
   ...
   private void extractTextPartAndTrimControlStructures( int endIndex, Mode mode ) {
      ...
      if ( mode == Mode.Condition || mode == Mode.ForLoop || mode == Mode.ForIndexLoop ) {
   ...
   private void extractHtmlCodePart() {
      if ( currentHtmlTag != null ) {
         ...
         if ( currentAttribute != null && currentAttribute.quoteCount < 2 ) {
            extract(templateCode, lastIndex, i,
                  ( depth, codePart ) -> visitor.onHtmlTagAttributeCodePart(depth, codePart, currentHtmlTag.name, currentAttribute.name));
            return;
         }
      }
   ...
   private void interceptHtmlTags() {
      if ( isOpeningHtmlTag() ) {
         ...
         if ( !currentHtmlTag.attributesProcessed && currentHtmlTag.isCurrentAttributeQuote(currentChar) ) {
            ...
            tagClosed = true;
         } else if ( !currentHtmlTag.attributesProcessed && !Character.isWhitespace(currentChar) && currentChar != '/'
               && currentHtmlTag.isCurrentAttributeComplete() ) {
   ...
   interface Mode {
      ...
      Mode ForIndexLoop     = new StatelessMode("ForIndexLoop");
   ...
   public static class HtmlTag implements gg.jte.html.HtmlTag {
      ...
      private static final Set<String> VOID_HTML_TAGS = new HashSet<>(
   ...
   public static class HtmlAttribute implements gg.jte.html.HtmlAttribute {
      ...
      private static final Set<String> BOOLEAN_HTML_ATTRIBUTES = new HashSet<>(
            Arrays.asList("allowfullscreen", "allowpaymentrequest", "async", "autofocus", "autoplay", "checked", "controls", "default", "disabled",
                  "formnovalidate", "hidden", "ismap", "itemscope", "loop", "multiple", "muted", "nomodule", "novalidate", "open", "playsinline", "readonly",
